...
-- on an 'Int' field.
data IntMinMaxQueue a = IntMinMaxQueue {-# UNPACK #-} !Size
                                       !MaxSize
                                       !(IntMap (NonEmpty a))
...
take lookup n m = go 0 m Map.empty
  where
    go :: Int -> IntMap (NonEmpty a) -> IntMap (NonEmpty a) -> IntMap (NonEmpty a)
    go sz mIn mOut
      | sz >= n = mOut
...
drop lookup n = go 0
  where
    go :: Int -> IntMap (NonEmpty a) -> IntMap (NonEmpty a)
    go sz mOut
      | sz >= n = mOut
